...
{
    public partial class frmWarehouseMaterialScanInWH2_v1o2 : Form
    {
        public string _locId = "102";     //warehouse ID
        ...
        public frmWarehouseMaterialScanInWH2_v1o2()
        {
            InitializeComponent();

            cls.SetDoubleBuffer(dgvLocList, true);
            cls.SetDoubleBuffer(dgvMatList, true);
            cls.SetDoubleBuffer(dgvScanList, true);
        }

        private void frmWarehouseMaterialScanInWH1_Load(object sender, EventArgs e)
        {
        ...
        public void init()
        {
            fnGetdate();
            initMatList();
            Fnc_Load_Vendor_List();
        }

        /**********************************************************/

        public void Fnc_Load_Vendor_List()
        {
            string
                sql = "V2o1_BASE_Warehouse_Material_ScanIn_Vendor_SelItem_Addnew";

            bool
                first_row = true;

            cls.Fnc_Data_To_Combobox(cbb_vendor, first_row, sql, "name", "idx");
        }
        ...
        public void fnBindMatList(string _locId)
        {
            try
            {
                ...
                ////dgvMatList.Columns[0].Width = 20 * _dgvMatListWidth / 100;    // idx
                //dgvMatList.Columns[1].Width = 70 * _dgvMatListWidth / 100;    // Machine
                //dgvMatList.Columns[2].Width = 30 * _dgvMatListWidth / 100;    // Line

                //dgvMatList.Columns[0].FillWeight = 20;    // idx
                dgvMatList.Columns[1].FillWeight = 70;    // Machine
                dgvMatList.Columns[2].FillWeight = 30;    // Line

                dgvMatList.Columns[0].Visible = false;
                dgvMatList.Columns[1].Visible = true;
                dgvMatList.Columns[2].Visible = true;

                cls.fnFormatDatagridview_FullWidth(dgvMatList, 12);
        ...
        public void fnBindWH_Location(string whId, string invId)
        {
            try
            {
                ...
                //dgvLocList.Columns[0].FillWeight = 20;    // idx
                dgvLocList.Columns[1].FillWeight = 40;    // Location
                dgvLocList.Columns[2].FillWeight = 20;    // Quantity
                dgvLocList.Columns[3].FillWeight = 40;    // IN Date
                //dgvLocList.Columns[4].FillWeight = 30;    // LocId
                ...
                cls.fnFormatDatagridview_FullWidth(dgvLocList, 12);
        ...
        public void fnGetQty(string prodId)
        {
            try
            {
                ...
                lblPCS.Enabled = (lblPCS.Text == "0") ? false : true;
                lblBOX.Enabled = (lblBOX.Text == "0") ? false : true;
                lblPAK.Enabled = (lblPAK.Text == "0") ? false : true;
                lblPAL.Enabled = (lblPAL.Text == "0") ? false : true;
        ...
        private void txtPackCode_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            string vendor = (cbb_vendor.SelectedIndex > 0) ? cbb_vendor.SelectedValue.ToString() : "";
            ...
            if (e.KeyCode == Keys.Enter)
            {
                if (packCode != "" && packCode != null)
                {
                    if (packCode.IndexOf("-") > 0)
                    {
                                    ...
                                    if (packQty > 0)
                                    {
                                        ...
                                        if (dtChk.Rows.Count > 0)
                                        {
                                            ...
                                            if (dsExist.Tables.Count == 0 || dsExist.Tables[0].Rows.Count == 0)
                                            {
                                                ...
                                                string sql = "V2o1_BASE_Warehouse_Material_ScanIn_AddItem_V1o3_Addnew";
                                                ...
                                                sParams[9].ParameterName = "@vendor_idx";
                                                sParams[9].Value = vendor;
                        ...
                        {
                            //lblMessage.Text = "Invalid packing code. Please try again.";
                            lblMessage.Text = "Chưa chọn vendor cho NVL";
                        }
                    }
                    else
                    {
        ...
        public void fnBindWH_Packing(string _locId, string _matId)
        {
            try
            {
                ...
                //dgvScanList.Columns[0].FillWeight = 20;    // inoutID
                //dgvScanList.Columns[1].FillWeight = 40;    // packingID
                dgvScanList.Columns[2].FillWeight = 20;    // packingCode
                //dgvScanList.Columns[3].FillWeight = 40;    // partId
                dgvScanList.Columns[4].FillWeight = 20;    // partName
                dgvScanList.Columns[5].FillWeight = 15;    // partNumber
                dgvScanList.Columns[6].FillWeight = 8;    // packingQty
                dgvScanList.Columns[7].FillWeight = 5;    // uom
                dgvScanList.Columns[8].FillWeight = 12;    // importLOT
                dgvScanList.Columns[9].FillWeight = 10;    // receiveDate
                //dgvScanList.Columns[10].FillWeight = 30;    // transferLocationID
                //dgvScanList.Columns[11].FillWeight = 30;    // transferLocation
                dgvScanList.Columns[12].FillWeight = 10;    // transferSublocate
                ...
                cls.fnFormatDatagridview_FullWidth(dgvScanList, 11);
        ...
        private void btnExport2Excel_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                export = cls.ExportToExcel(dgvScanList, "CKD " + String.Format("{0:dd-MM-yyyy}", DateTime.Now));
