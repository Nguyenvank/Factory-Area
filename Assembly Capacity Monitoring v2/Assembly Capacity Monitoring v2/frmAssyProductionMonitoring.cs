...
{
    public partial class frmAssyProductionMonitoring : Form
    {
        public int _dgvAssyMonitorWidth;
        public int indexView = 1;

        public frmAssyProductionMonitoring()
        {
            InitializeComponent();
        }

        private void frmAssyProductionMonitoring_Load(object sender, EventArgs e)
        {
            ...
            if (check.IsConnectedToInternet())
            {
                ...
                switch(indexView)
                {
                    case 1:
                        fnView01();
                        fnCheckStatus01();
                        toolStripStatusLabel3.Text = " SCREEN 01 ";
                        break;
                    case 2:
                        fnView02();
                        fnCheckStatus02();
                        toolStripStatusLabel3.Text = " SCREEN 02 ";
                        break;
                }
            }
            fnGetdate();
        ...
        public void fnGetdate()
        {
            try
            {
                if (check.IsConnectedToInternet())
                {
                    toolStripStatusLabel4.Text = cls.fnGetDate("SD") + " " + cls.fnGetDate("CT");
                    toolStripStatusLabel4.ForeColor = Color.Black;
                }
                else
                {
                    toolStripStatusLabel4.Text = String.Format("{0:dd/MM/yyyy}", DateTime.Now) + " " + String.Format("{0:HH:mm:ss}", DateTime.Now);
        ...
        public void fnView02()
        {
            try
            {
                ...
                dgvAssyMonitor.Columns[27].Width = 24 * _dgvAssyMonitorWidth / 100;   // [PicName]
        ...
        private void sceen01ToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            if (indexView != 1)
            {
                indexView = 1;
                dgvAssyMonitor.DataSource = "";
                dgvAssyMonitor.Refresh();
            }
        }

        private void sceen01ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (indexView != 2)
            {
                indexView = 2;
        ...
        public void fnIdleTimeStart (string orderID)
        {
            try
            {
