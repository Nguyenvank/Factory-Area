...
namespace Fan_1
{
    public partial class frmAssyDispenser_v1o1 : Form
    {
        public int run = 1;
        ...
        private void frmAssyDispenser_v1o1_Load(object sender, EventArgs e)
        {
            _dt = DateTime.Now;

            string value = "";
            string filename = "";
            string prefix_Year = _dt.Year.ToString();
            string prefix_Month = String.Format("{0:00}", _dt.Month);
            string prefix_Day = String.Format("{0:00}", _dt.Day);
        ...
        private void TxtDI01SCN_TextChanged(object sender, EventArgs e)
        {
            ...
            if (txtDI01SCN.Text.ToLower() == "true")
            {
                ...
                lstBox01 = "Line01, " + String.Format("{0:dd/MM/yyyy HH:mm:ss}", _dt);
                lstNGLog.Items.Add(lstBox01);

                //txtDI01SCN.BackColor = Color.LightPink;
            }
            //else
            //{
            //    txtDI01SCN.BackColor = Color.FromKnownColor(KnownColor.Control);
            //}
            txtDI01SCN.BackColor = (txtDI01SCN.Text.ToLower() == "true") ? Color.LightPink : Color.FromKnownColor(KnownColor.Control);
        ...
        private void TxtDI02SCN_TextChanged(object sender, EventArgs e)
        {
            ...
            if (txtDI02SCN.Text.ToLower() == "true")
            {
                ...
                lstBox02 = "Line02, " + String.Format("{0:dd/MM/yyyy HH:mm:ss}", _dt);
                lstNGLog.Items.Add(lstBox02);

                //txtDI02SCN.BackColor = Color.LightPink;
            }
            //else
            //{
            //    txtDI02SCN.BackColor = Color.FromKnownColor(KnownColor.Control);
            //}
            txtDI02SCN.BackColor = (txtDI02SCN.Text.ToLower() == "true") ? Color.LightPink : Color.FromKnownColor(KnownColor.Control);
        ...
        private void frmAssyDispenser_v1o1_Resize(object sender, EventArgs e)
        {
        ...
        public void Fnc_Prepair_NG_Log()
        {
            ...
            {
                string fName = "", lst = "";
                foreach (object lstText in lstNGLog.Items)
                {
                    lst += lstText.ToString() + "\r\n";
                }
        ...
        {
            string logNG = "";
            foreach(object item in lstNGLog.Items)
            {
                logNG += item.ToString() + "\r\n";
            }
            Clipboard.SetText(logNG);
            MessageBox.Show("NG Log copied!\r\n(Paste to any editable program that you need)");
        }
