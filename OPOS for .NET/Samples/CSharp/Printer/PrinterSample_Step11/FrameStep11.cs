...
namespace PrinterSample_Step11
{
	/// <summary>
	...
	public class FrameStep11 : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.grpReceipt.Size = new System.Drawing.Size(192, 144);
			...
			// btnAsync
			// 
			this.btnAsync.Location = new System.Drawing.Point(20, 66);
			...
			this.grpSlp.Size = new System.Drawing.Size(192, 84);
			...
			// btnPanelSwitch
			// 
			this.btnPanelSwitch.Location = new System.Drawing.Point(38, 100);
			...
			// btnPrintCode128
			// 
			this.btnPrintCode128.Location = new System.Drawing.Point(38, 64);
			...
			// btnSupportFunction
			// 
			this.btnSupportFunction.Location = new System.Drawing.Point(38, 28);
			...
			this.Name = "FrameStep11";
			this.Text = "Step 11  Use the EPSON original methods";
			...
			try
			{
				if (m_Printer.CapRecPresent == true)
				{
					...
					{			
						...
						{
							MessageBox.Show("Cannot use a POS Printer.","Printer_SampleStep11"
						...
						catch(PosControlException)
						{
							...
							dialogResult = MessageBox.Show("Failed to output to printer.\n\nRetry?"
								,"Printer_SampleStep11",MessageBoxButtons.AbortRetryIgnore);
				...
				{
					MessageBox.Show("Cannot use a Receipt Stateion.","Printer_SampleStep11"
						,MessageBoxButtons.OK,MessageBoxIcon.Warning);
					return;
				}
			}
		...
		private void btnReceipt_Click(object sender, System.EventArgs e)
		{
					...
					catch(PosControlException ex )
					{
						if(ex.ErrorCode == ErrorCode.Illegal && ex.ErrorCodeExtended == 1004)
						{
							MessageBox.Show("Unable to print receipt.\n" + GetErrorCode(ex)
								,"Printer_SampleStep11",MessageBoxButtons.OK,MessageBoxIcon.Warning);
			...
			catch(PosControlException ex )
			{
				// Clear the buffered data since the buffer retains print data when an error occurs during printing.
				m_Printer.ClearOutput();
				MessageBox.Show("Unable to print receipt.\n" + GetErrorCode(ex),"Printer_SampleStep11",
					MessageBoxButtons.OK,MessageBoxIcon.Warning);
		...
		private void btnSupportFunction_Click(object sender, System.EventArgs e)
		{
			...
			{
				MessageBox.Show("This function is disable to use.","PrinterSample_Step11");
			}
			//<<<step11>>--End
		}
		...
		private void btnPrintCode128_Click(object sender, System.EventArgs e)
		{
			...
			{
				MessageBox.Show("DirectIO Code128 failed.","PrinterSample_Step11");
		...
		private bool WaitForInsertion()
		{
			...
			while(true)
			{
				...
				catch(PosControlException pex)
				{
					...
					{
						dialogResult =  MessageBox.Show("Please insert a form.","PrinterSample_Step11"
					...
					else
					{
						...
						MessageBox.Show(strMessage,"PrinterSample_Step11");
						bInsertion = false;
					}
				}
		...
		private void frmStep11_Load(object sender, System.EventArgs e)
		{
			...
			string strFilePath = strCurDir.Substring(0,strCurDir.LastIndexOf("Step11") + "Step11\\".Length);
			...
			{
				MessageBox.Show("Failed to get device information.","PrinterSample_Step11");
				//Nothing can be used.
				ChangeButtonStatus();
				return;
			}
			...
			{
				MessageBox.Show("Failed to create instance.","PrinterSample_Step11");
				//Nothing can be used.
				ChangeButtonStatus();
				return;
			...
			{
				MessageBox.Show("Failed to open the device.","PrinterSample_Step11");
			...
			{
				MessageBox.Show("Failed to claim the device.","PrinterSample_Step11");
			...
			{
				MessageBox.Show("Disable to use the device.","PrinterSample_Step11");
				//Nothing can be used.
				ChangeButtonStatus();
				return;
			}
            ...
            if (m_Printer.CapRecBitmap == true)
            {
                ...
                {
                    MessageBox.Show("Failed to set bitmap.", "Printer_SampleStep11"
                            , MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
			//<<<step3>>>--End
