...
{
    public partial class frmFMBScanBarcode_v2o6 : Form
    {
        ...
        private void frmFMBScanBarcode_v2o6_Load(object sender, EventArgs e)
        {
            _dt = DateTime.Now;
            init();
            //init_FMB_NG();
        ...
        public void Fnc_Load_Confirm()
        {
            ...
            try
            {
                ...
                _ds = cls.ExecuteDataSet(__sql);
                _tbl_cnt = _ds.Tables.Count;
                _row_cnt = _ds.Tables[0].Rows.Count;

                if (_tbl_cnt > 0 && _row_cnt > 0)
                {
                    err_cd = _ds.Tables[0].Rows[0][2].ToString();
                    err_msg = _ds.Tables[0].Rows[0][4].ToString();
                    fmb_cd = _ds.Tables[0].Rows[0][5].ToString();
                    accept = _ds.Tables[0].Rows[0][7].ToString();

                    _err_cd = (err_cd.Length > 0) ? Convert.ToInt32(err_cd) : 0;
                    _accept = (accept.ToLower() == "true") ? true : false;
                    ...
                    switch (_accept)
                    {
                        case true:
                            break;
                        case false:
                            frmFMBScanBarcode_v2o6_Warning_v1o0 form = new frmFMBScanBarcode_v2o6_Warning_v1o0(_err_cd, err_msg, fmb_cd);
                            form.ShowDialog();
                            break;
                    }
                }
        ...
        public void Fnc_Load_Init_Selection()
        {
            ...
            chk_FMB_Mat.Enabled = txt_FMB_IN.Enabled = false;
        }
        ...
        public void Fnc_Load_FIFO_Items()
        {
            try
            {
                ...
                cls.fnFormatDatagridview_FullWidth(dgv_CMF_UnFiFo, 50, 30, 90);
        ...
        private void btn_epdm50_Click(object sender, EventArgs e)
        {
            frmFMBScanBarcode_v2o6_EPDM50 epdm50 = new frmFMBScanBarcode_v2o6_EPDM50();
            epdm50.ShowDialog();

            init_FMB_IN_List();
            Fnc_Load_Init_Selection();
        }
                        ...
                        if (packKind == "PRO" || packKind == "BOX")
                        {
                            if (packType == "069")
                            {
                                ...
                                if (ds.Tables.Count == 0 || ds.Tables[0].Rows.Count == 0)
                                {
                                    ...
                                    frmFMBScanBarcode_v2o6_CMF cmf = new frmFMBScanBarcode_v2o6_CMF(packing, matLot, pakWeight, matIDx);
                                    cmf.ShowDialog();
        ...
        public void init_FMB_OUT_Machine()
        {
            ...
            else if (rdb_FMB_R23.Checked)
            {
                _mchNum = "23";
            }
            else if (rdb_FMB_R24.Checked)
            {
                _mchNum = "24";
            }
            else if (rdb_FMB_R25.Checked)
            {
                _mchNum = "25";
            }
                ...
                try
                {
                    ...
                    if (packing.Length >= 12)
                    {
                        ...
                        if (packKind == "PRO" || packKind == "BOX")
                        {
                            if (packType == "069" || packType == "CMF")
                            {
                                ...
                                if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                                {
                                    ...
                                    if (_coolTime >= 21600)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            frmFMBScanBarcode_v2o6_Warning_v1o0 form = new frmFMBScanBarcode_v2o6_Warning_v1o0(1, msg, packing);
                                            form.ShowDialog();
                                    ...
                                    else
                                    {
                                        ...
                                        _msgText = "Chưa đủ thời gian ủ.\r\n\r\nMã kiện này chưa đủ thời gian ủ trên hệ thống (" + String.Format("{0:0.0}", _coolTime / 3600) + " h) nên sẽ không xuất ra máy sản xuất được.";
                                        _msgType = 2;

                                        frmFMBScanBarcode_v2o6_Warning_v1o0 form = new frmFMBScanBarcode_v2o6_Warning_v1o0(2, _msgText, packing);
                                        form.ShowDialog();
                                    }
                                }
                                else
                                {
                                    ...
                                    _msgText = "Mã kiện này không có trên hệ thống.\r\n\r\nVui lòng kiểm tra và thử lại hoặc thay tem khác.";
                                    _msgType = 2;

                                    frmFMBScanBarcode_v2o6_Warning_v1o0 form = new frmFMBScanBarcode_v2o6_Warning_v1o0(3, _msgText, packing);
                                    form.ShowDialog();
                                }
                            }
                            else
                            {
                                ...
                                _msgText = "Mã kiện không đúng loại cho FMB.\r\n\r\nVui lòng kiểm tra và thử lại hoặc thay tem khác.";
                                _msgType = 2;

                                frmFMBScanBarcode_v2o6_Warning_v1o0 form = new frmFMBScanBarcode_v2o6_Warning_v1o0(4, _msgText, packing);
                                form.ShowDialog();
                            }
                        }
                        else
                        {
                            ...
                            frmFMBScanBarcode_v2o6_Warning_v1o0 form = new frmFMBScanBarcode_v2o6_Warning_v1o0(5, _msgText, packing);
                            form.ShowDialog();
                        }
                    }
                    else
                    {
                        ...
                        _msgText = "Mã kiện không đúng.\r\n\r\nVui lòng kiểm tra và thử lại hoặc thay tem khác.";
                        _msgType = 2;

                        frmFMBScanBarcode_v2o6_Warning_v1o0 form = new frmFMBScanBarcode_v2o6_Warning_v1o0(6, _msgText, packing);
                        form.ShowDialog();
                    }
        ...
        {
            frmFMBScanBarcode_v2o6_NGCart cart = new frmFMBScanBarcode_v2o6_NGCart();
            cart.ShowDialog();
        }
        ...
        private void frmFMBScanBarcode_v2o6_Shown(object sender, EventArgs e)
        {
            Fnc_Load_Init_Selection();
        }
        ...
        private void rdb_FMB_R23_MouseClick(object sender, MouseEventArgs e)
        {
            init_FMB_OUT_Machine();
        }

        private void rdb_FMB_R24_MouseClick(object sender, MouseEventArgs e)
        {
            init_FMB_OUT_Machine();
        }

        private void rdb_FMB_R25_MouseClick(object sender, MouseEventArgs e)
        {
            init_FMB_OUT_Machine();
        }
