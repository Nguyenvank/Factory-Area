...
{
    public partial class frmInternalUpdateBoxcode : Form
    {
        ...
        public string _packingIDx = "", _packingCode = "", _packingLoc = "", _packingDate = "", _packingLot = "", _packingQty = "";
        public int _rowPackingIndex = 0;
        ...
        public frmInternalUpdateBoxcode()
        {
            InitializeComponent();
        ...
        private void frmInternalUpdateBoxcode_Load(object sender, EventArgs e)
        {
        ...
        public void fnPackCOD(string pack)
        {
            ...
            txtPacking_Filter.Enabled = false;
            txtPacking_Filter.Text = "";
        ...
        private void tlpIN_Packing_Click(object sender, EventArgs e)
        {
            txtIN_Packing.Focus();
        }

        private void tlpIN_Packing_MouseClick(object sender, MouseEventArgs e)
        {
            txtIN_Packing.Focus();
        }
        ...
        {
            txtPacking_Filter.Enabled = true;
            txtPacking_Filter.BackColor = Color.FromKnownColor(KnownColor.Info);

            init_Del_Filter();
            init_Del_List();
        }

        public void init_Del_Filter()
        {
            
        }

        public void init_Del_List()
        {
            string partIDx = _partIDx;
            string sql = "V2_BASE_Inventory_ScanIn_Delete_Box_SelItem_V1o1_Addnew";
            ...
            //dgvPacking_List.Columns[0].Width = 25 * _dgvPacking_List_Width / 100;    // boxId
            dgvPacking_List.Columns[1].Width = 55 * _dgvPacking_List_Width / 100;    // boxcode
            //dgvPacking_List.Columns[2].Width = 25 * _dgvPacking_List_Width / 100;    // boxsublocate
            //dgvPacking_List.Columns[3].Width = 25 * _dgvPacking_List_Width / 100;    // packingdate
            dgvPacking_List.Columns[4].Width = 30 * _dgvPacking_List_Width / 100;    // boxLOT
            dgvPacking_List.Columns[5].Width = 15 * _dgvPacking_List_Width / 100;    // boxquantity

            dgvPacking_List.Columns[0].Visible = false;
            dgvPacking_List.Columns[1].Visible = true;
            dgvPacking_List.Columns[2].Visible = false;
            dgvPacking_List.Columns[3].Visible = false;
            dgvPacking_List.Columns[4].Visible = true;
            dgvPacking_List.Columns[5].Visible = true;

            dgvPacking_List.Columns[3].DefaultCellStyle.Format = "dd/MM/yyyy HH:mm";
            cls.fnFormatDatagridview(dgvPacking_List, 11, 30);
        }

        private void dgvPacking_List_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
        ...
        private void dgvPacking_List_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                row = dgvPacking_List.Rows[e.RowIndex];

                string boxIDx = row.Cells[0].Value.ToString();
                string boxCode = row.Cells[1].Value.ToString();
                string boxLoc = row.Cells[2].Value.ToString();
                string boxDate = row.Cells[3].Value.ToString();
                string boxLot = row.Cells[4].Value.ToString();
                string boxQty = row.Cells[5].Value.ToString();

                _packingIDx = boxIDx;
                _packingCode = boxCode;
                _packingLoc = boxLoc;
                _packingDate = boxDate;
                _packingLot = boxLot;
                _packingQty = boxQty;
        ...
        private void dgvPacking_List_CellMouseUp(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                dgvPacking_List.ClearSelection();
                dgvPacking_List.Rows[e.RowIndex].Selected = true;
                _rowPackingIndex = e.RowIndex;
                dgvPacking_List.CurrentCell = dgvPacking_List.Rows[e.RowIndex].Cells[1];
                contextMenuStrip1.Show(this.dgvPacking_List, e.Location);
                contextMenuStrip1.Show(Cursor.Position);
            }
        }

        private void txtPacking_Filter_TextChanged(object sender, EventArgs e)
        {
            BindingSource bs = new BindingSource();
            bs.DataSource = dgvPacking_List.DataSource;
            bs.Filter = string.Format("CONVERT([" + dgvPacking_List.Columns[1].DataPropertyName + "], System.String) like '%" + txtPacking_Filter.Text.Replace("'", "''") + "%'");
            dgvPacking_List.DataSource = bs;
        }

        private void xóaKiệnĐangChọnToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                string boxIDx = _packingIDx;
                string boxCode = _packingCode;
                string boxLoc = _packingLoc;
                string boxDate = _packingDate;
                string boxLot = _packingLot;
                string boxQty = _packingQty;
                string prodIDx = _partIDx;

                string msg = "";
                msg = "DỮ LIỆU ĐÃ XÓA LÀ KHÔNG THỂ PHỤC HỒI\r\n\r\n";
                msg += "Kiện hàng hiện tại:\r\n";
                msg += "Mã kiện: " + boxCode + "\r\n";
                msg += "Vị trí: " + boxLoc + "\r\n";
                msg += "Ngày sx: " + String.Format("{0:dd/MM/yyyy HH:mm}", Convert.ToDateTime(boxDate)) + "\r\n";
                msg += "Số LOT: " + boxLot + "\r\n";
                msg += "Số lượng: " + boxQty + "\r\n";
                msg += "\r\n";
                msg += "Có chắc là bạn vẫn muốn XÓA kiện này?";

                DialogResult dialog = MessageBox.Show(msg, cls.appName(), MessageBoxButtons.YesNo);
                if (dialog == DialogResult.Yes)
                {
                    string sql = "V2_BASE_Inventory_ScanIn_Delete_Box_DelItem_V1o1_Addnew";
                    SqlParameter[] sParams = new SqlParameter[5]; // Parameter count

                    sParams[0] = new SqlParameter();
                    sParams[0].SqlDbType = SqlDbType.Int;
                    sParams[0].ParameterName = "@boxIDx";
                    sParams[0].Value = boxIDx;

                    sParams[1] = new SqlParameter();
                    sParams[1].SqlDbType = SqlDbType.VarChar;
                    sParams[1].ParameterName = "@boxCode";
                    sParams[1].Value = boxCode;

                    sParams[2] = new SqlParameter();
                    sParams[2].SqlDbType = SqlDbType.VarChar;
                    sParams[2].ParameterName = "@boxLoc";
                    sParams[2].Value = boxLoc;

                    sParams[3] = new SqlParameter();
                    sParams[3].SqlDbType = SqlDbType.Int;
                    sParams[3].ParameterName = "@boxQty";
                    sParams[3].Value = boxQty;

                    sParams[4] = new SqlParameter();
                    sParams[4].SqlDbType = SqlDbType.Int;
                    sParams[4].ParameterName = "@prodIDx";
                    sParams[4].Value = prodIDx;

                    cls.fnUpdDel(sql, sParams);

                    _msgText = "Xóa kiện " + boxCode + " thành công";
                    _msgType = 1;
                }
            }
